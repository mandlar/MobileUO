...
namespace ClassicUO.Renderer
{
    class TextureAtlas : IDisposable
    {
        ...
        public unsafe Texture2D AddSprite(Span<uint> pixels, int width, int height, out Rectangle pr)
        {
            // MobileUO: using TextureAtlas isn't working correctly, revert to old method
            // MobileUO: TODO: figure out how to get this working with the TextureAtlas
            Texture2D texture = new Texture2D(Client.Game.GraphicsDevice, width, height);
            //texture.SetData(pixels.ToArray(), 0, width * height);
